...
<?php if ($features) : ?>
    <section class="py-[40px] lg:py-[80px]" data-x-data="{activeIndex: 0, numberOfItems: <?= count($features); ?>}">
        <div class="max-w-[1200px] mx-auto px-[34px] lg:px-[60px] relative box-border">
            <button aria-label="Previous Slide" class="hidden lg:flex justify-center items-center absolute top-[235px] left-[4px] w-[40px] h-[40px] rounded-full bg-transparent border border-[#393A40] border-solid text-[#393A40] hover:border-[rgba(97,99,107,0.3)] hover:text-[rgba(97,99,107,0.3)] transition-colors"
                            ...
                            <div style="order: <?= (in_array($index, array(2, 3))) ? $index === 3 ? 3 : 4 : $index + 1; ?>">
                                <div class="pt-[100%] relative">
                                    <button data-x-on:click="activeIndex = <?= $index; ?>"
                                            class="p-0 bg-transparent border-0 absolute inset-0 w-full h-full lg:w-[calc(100%+30px)] lg:h-[calc(100%+30px)] rounded-full lg:m-[-13px] overflow-hidden"
