...
<?php if ($features) : ?>
    <section class="py-[80px]" x-data="{activeIndex: 0, numberOfItems: <?= count($features); ?>}">
        <div class="max-w-[1200px] mx-auto px-[34px] lg:px-[60px] relative box-border">
            <button class="hidden lg:flex justify-center items-center absolute top-[50%] translate-y-[-50%] left-[4px] w-[40px] h-[40px] rounded-full bg-transparent border border-[#393A40] border-solid text-[#393A40] hover:border-[rgba(97,99,107,0.3)] hover:text-[rgba(97,99,107,0.3)] transition-colors"
                    @click="activeIndex = activeIndex === 0 ? numberOfItems - 1 : activeIndex - 1">
            ...
            <button class="hidden lg:flex justify-center items-center absolute top-[50%] translate-y-[-50%] right-[4px] w-[40px] h-[40px] rounded-full bg-transparent border border-[#393A40] border-solid text-[#393A40] hover:border-[rgba(97,99,107,0.3)] hover:text-[rgba(97,99,107,0.3)] transition-colors"
                    @click="activeIndex = activeIndex === numberOfItems - 1 ? 0 : activeIndex + 1">
                <svg width="9" height="16" viewBox="0 0 9 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <div style="order: <?= (in_array($index, array(2, 3))) ? $index === 3 ? 3 : 4 : $index + 1; ?>">
                                <div class="pt-[100%] relative">
                                    <button @click="activeIndex = <?= $index; ?>"
                                            ...
                                            :class="{'z-30' : activeIndex === <?= $index; ?>, 'z-20' : [0, 2].includes(<?= $index; ?>) && activeIndex !== <?= $index; ?> }"
                                    >
                                        <!--not active-->
                                        ...
                                        <span style="background: linear-gradient(90deg, rgba(<?= $feature['title_gradient_color_start']['red']; ?>, <?= $feature['title_gradient_color_start']['green']; ?>, <?= $feature['title_gradient_color_start']['blue']; ?>, 0.5) 0.6%, rgba(<?= $feature['title_gradient_color_end']['red']; ?>, <?= $feature['title_gradient_color_end']['green']; ?>, <?= $feature['title_gradient_color_end']['blue']; ?>, 0.2) 75%), #FFFFFF;"
                                              class="font-poppins text-transparent !bg-clip-text text-[16px] lg:text-[28px] leading-[1.4] font-[600] tracking-[0.05em] uppercase text-center"><?= $feature['title']; ?></span>
                                        </div>
                                        <!--!active-->
                                    </button>
                                </div>
